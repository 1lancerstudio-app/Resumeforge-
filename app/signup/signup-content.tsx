        ...
        {step === 2 && (
          <div className="flex flex-col flex-1 justify-center max-w-sm">
            ...
            <Button
              type="button"
              variant="outline"
              className="w-full rounded-full h-12 text-sm border-border hover:bg-foreground hover:text-background transition-colors gap-3"
              onClick={() => {
                const clientId = process.env.NEXT_PUBLIC_GITHUB_CLIENT_ID;
                const redirectUri = `${window.location.origin}/api/github/callback`;
                if (clientId) {
                  window.location.href = `https://github.com/login/oauth/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent("read:user,public_repo")}`;
                } else {
